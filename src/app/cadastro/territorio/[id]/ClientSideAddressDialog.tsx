...
import type { House, BlockAddress } from "./type"
...
export const AddressDialog = ({ address, blockId }: AddressDialogProps) => {
    ...
    return (
        <Dialog open={opened} onOpenChange={setOpened}>
            ...
            <DialogContent className="sm:max-w-[95vw] md:max-w-[90vw] lg:max-w-[80vw] xl:max-w-[70vw] max-h-[90vh] overflow-hidden flex flex-col">
                <DialogHeader>
                ...
                <DialogDescription className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <span>
                        Essa rua possui {houses.length} casa{houses.length > 1 ? "s" : ""}
                    </span>
                    <Button
                        disabled={houses?.some((house) => house?.id === 0)}
                        onClick={handleAddHouse}
                        variant="outline"
                        className="flex items-center gap-2 bg-primary text-white hover:text-primary hover:border-primary shadow-md hover:shadow-sm w-full sm:w-auto"
                    >
                        Adicionar casa
                        <PlusIcon />
                    </Button>
                                ...
                                {houses.map((house) => (
                                    ...
                                    >
                                        <TableCell className={`w-[40px] ${editingHouse?.id === 0 ? "" : "cursor-move"}`}>
                                            <svg
                                                className={`w-6 h-6 text-gray-500 ${editingHouse?.id === 0 ? "invisible" : ""}`}
                                                fill="none"
                                                stroke="currentColor"
                                                viewBox="0 0 24 24"
                                            >
                                        ...
                                        <TableCell className="w-4/12 max-w-[120px]">
                                            {editingHouse?.id === house.id ? (
                                                <Input
                                                    className="bg-white"
                                                    type="text"
                                                    value={editingHouse.number}
                                                    onChange={(e) => setEditingHouse({ ...editingHouse, number: e.target.value })}
                                                />
                                            ) : (
                                                <span className="truncate block">{house.number}</span>
                                            )}
                                        </TableCell>
                                        <TableCell className="w-3/12 max-w-[120px]">
                                            {editingHouse?.id === house.id ? (
                                                <Select
                                                    value={editingHouse.legend}
                                                    onValueChange={(value) => setEditingHouse({ ...editingHouse, legend: value })}
                                                >
                                                    ...
                                                    <SelectContent>
                                                        {legendas.map((legend) => (
                                                            <SelectItem key={legend.name} value={legend.name}>
                                                                {legend.name}
                                                            </SelectItem>
                                                        ))}
                                                    </SelectContent>
                                                ...
                                                <span className="truncate block">{house.legend}</span>
                                            )}
                                        </TableCell>
                                        <TableCell className="w-2/12">
                                            {editingHouse?.id === house.id ? (
                                                <Select
                                                    value={editingHouse.dontVisit ? "true" : "false"}
                                                    onValueChange={(value) => setEditingHouse({ ...editingHouse, dontVisit: value === "true" })}
                                                >
                                                    <SelectTrigger className="bg-white">
                                            ...
                                            <div className="flex gap-1 justify-center">
                                                {editingHouse && editingHouse.id !== house.id ? (
                                                    <Button
                                                        ...
                                                        variant="outline"
                                                        size="icon"
                                                        title={"Edição em andamento na casa " + house.number}
                                                        className="cursor-not-allowed"
                                                    >
                                                        <PencilIcon className="text-blue-500 opacity-90 animate-pulse" />
                                                    </Button>
                                                ) : (
                                                    <>
                                                        <Button
                                                            variant="outline"
                                                            disabled={editingHouse && (!editingHouse?.number || !editingHouse?.legend)}
                                                            size="icon"
                                                            onClick={() => handleEditHouse(house)}
                                                            title={editingHouse?.id === house.id ? "Salvar alteração" : "Editar casa"}
                                                        >
                                                            {editingHouse?.id === house.id ? (
                                        ...
                                        <TableCell className="w-3/12">
                                            <div className="flex gap-1 justify-center">
                                                {deletingHouse && deletingHouse.id === house.id && (
                                                    <>
                                                        ...
                                                        <Button
                                                            ...
                                                            size="icon"
                                                            title={"Cancelar"}
                                                            onClick={() => setDeletingHouse(undefined)}
                                                        >
                                                            <XIcon className="text-red-500" />
                                                        </Button>
                                                    </>
                                                ...
                                                <Button
                                                    ...
                                                    className={`${(deletingHouse && deletingHouse.id === house.id) || (editingHouse?.id === house.id && editingHouse?.id === 0) ? "hidden" : ""}`}
                                                >
